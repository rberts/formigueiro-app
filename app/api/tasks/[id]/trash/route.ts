...
export async function POST(
  ...
  if (taskError) {
    console.error('[POST /api/tasks/:id/trash] erro ao validar tarefa:', taskError);
  ...
  const { data: updatedTask, error: updateError } = await supabase
    .from('tasks')
    .update({ visibility: 'trashed' })
    .eq('id', params.id)
    .select('id, project_id, title, description, status, visibility, start_date, due_date, created_by, created_at, updated_at')
    .maybeSingle();

  if (updateError) {
    console.error('[POST /api/tasks/:id/trash] erro ao mover tarefa para lixeira:', updateError);
    return errorResponse(
      'DB_ERROR',
      'Erro ao mover tarefa para lixeira.',
      500,
